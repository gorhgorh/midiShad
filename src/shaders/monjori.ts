...
const fragmentShader = /* glsl */ `
  ...
  void main() {
    ...
    for (float i = 0.0; i < 15.0; i++) {
      float x = sin(t + i / u_distortion + u_offset) * u_complexity;
      a += 1.0 / length(
        vec2(
...
export const monjori: ShaderDefinition = {
  id: 'monjori',
  name: 'Monjori',
  note: 'Parameterized Monjori — organic glowing grid distortion',
  credit: 'Original shader by Mic (Shadertoy)',
  vertexShader,
  fragmentShader,
  params: [
    { name: 'u_timeScale', label: 'Time Speed', min: 0, max: 100, default: 40, invert: true },
    ...
    { name: 'u_brightness', label: 'Brightness', min: 100, max: 800, default: 350, invert: true },
    { name: 'u_offset', label: 'Offset', min: -1, max: 1, default: 0 },
  ],
}
